...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await register(form);
      // Send the user to whichever workspace matches their role
      if (form.role === "ADMIN") navigate("/admin");
      else if (form.role === "TRAVEL_AGENT") navigate("/agent");
        ...
        <div className="auth-form-inner auth-form-inner-wide">
          ...
          <form onSubmit={handleSubmit} className="auth-form">
            <div className="auth-role-picker">
              <span className="auth-role-label">Sign up as</span>
              <div className="auth-role-options">
                {[
                  { id: "CUSTOMER", label: "Traveler", hint: "Book trips for yourself or your group" },
                  { id: "TRAVEL_AGENT", label: "Travel Agent", hint: "Manage packages and customer bookings" },
                  { id: "ADMIN", label: "Administrator", hint: "Full system oversight" },
                ].map((opt) => (
                  <button
                    type="button"
                    key={opt.id}
                    className={`auth-role-opt ${form.role === opt.id ? "is-active" : ""}`}
            ...
            {form.role !== "CUSTOMER" && (
              <div className="auth-staff-note">
                Staff accounts skip the traveler-only fields. You can configure permissions later from the Admin console.
              </div>
            )}
